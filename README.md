...
- `config/execution-adapters.json` adapter mapping for execution bridging
...
### Execution bridge
```bash
python3 scripts/execute_routed_task.py --format text "按方案推进，直接干"
python3 scripts/execute_routed_task.py --execute --format text "打开网页并截图，顺便点一下按钮"
python3 scripts/execute_routed_task.py --format json --output reports/execute-routed-task.json --output-format json "帮我搜索一下这个技术方案的最新资料"
```
...
## Execution bridge highlights
`execute_routed_task.py` adds:
...
## Documentation map
- `docs/VIBE_FEATURES_SUMMARY.md`: full capability summary across phases 1-20
